...
\todo{This seems not quite correct. Something like this:
In Chapter~\ref{ch:pes-statistics}, we explored how photoemission statistics are shaped by the characteristics of the light source, which exhibit well-defined statistical properties. However, with multidimensional detection schemes capturing millions of voxels, modeling the complexity of the measurement process becomes infeasible. Furthermore, such models would rely heavily on pre-existing knowledge of the materials under investigation, which are often the primary subject of inquiry. While studying the physical process in detail is fascinating, it may not be practical for tasks such as image restoration.

Traditional methods like \gls{BM3D} encode prior knowledge about the corruption process, such as the assumption of \gls{AWGN} or Poisson noise (see \cref{sec:poisson-noise-model}), into their algorithmic structure. Machine learning approaches, by contrast, offer the flexibility to learn such relationships from data. However, it is important to note that they do not operate entirely free of prior assumptions. For instance, in linear regression or generalized linear models (GLMs), prior knowledge is encoded through the choice of feature functions and assumptions about the underlying relationships. Similarly, in deep learning, architectures like convolutional neural networks (CNNs) implicitly encode priors, such as spatial locality and translation invariance, into their structure.

What sets machine learning apart is its ability to capture complex, nonlinear mappings and adapt to data-driven patterns that might not be explicitly designed beforehand. This flexibility allows machine learning models to extract meaningful information from incomplete or noisy measurements, even in scenarios where the explicit modeling of noise or corruption processes is infeasible.}
