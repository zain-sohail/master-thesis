...
\section{Noise2Noise}
In the classical supervised-learning setting for image restoration, we assume access to clean target images for training. However, in many practical scenarios such as our \gls{MPES} data, we only have access to a noisy realization of the latent clean image. 

\citeauthor{lehtinenNoise2NoiseLearningImage2018} demonstrated that by using only noisy observations for both inputs and targets, it is possible to train a neural network and learn a (restoration) mapping to the underlying clean signal \cite{lehtinenNoise2NoiseLearningImage2018}. Using the $L_2$ loss, a training set $\mathcal{S}^\prime = \{(x_1, x_1^\prime), \dots, (x_n, x_n^\prime)\}$ where $\mathcal{S}^\prime \sim \mathcal{D}^n$, $\mathcal{S}^\prime$ has noisy realizations of the same underlying signal $y \sim \mathcal{D}$, the \gls{ERM} defined in \cref{sec:erm} becomes:
\begin{equation}
    \hat{h} = \argmin_{h \in \mathcal{H}} \frac{1}{\lvert \mathcal{S^\prime} \rvert} \sum_{(x, x^\prime) \in \mathcal{S}^\prime} (h(x; \theta) - x^\prime)^2
\end{equation}
which only works using the $L_2$ loss. For zero-mean noise\footnote{We have discussed this for the case of Poisson noise in \cref{sec:poisson-noise-model}, specifically \cref{eq:poisson-noise,eq:zero-mean-noise}.}, the $L_2$ loss recovers the clean signal on expectation.
\begin{equation}
