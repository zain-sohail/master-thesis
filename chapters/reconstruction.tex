Detached from a noise model, the goal of image restoration is to estimate the true image $Y$ \todo{It's always good to state where objects come from, i.e., in which spaces they are. Here it could be $Y\in\mathbb{R}^{n_1\times\cdot\times n_d}$ if $Y$ is a $d$-dimensional discrete real valued image.} from an incomplete or corrupted (noisy) observation $X$\todo{$\in$?}. Let $\mathbf{i} = [i_1, i_2, \dots, i_d]^T$\todo{$\in$? In any case, the set of all possible indices should have a name, e.g., $\mathcal{I}$.} be a column vector that defines the coordinates of each voxel in a \textit{d}-dimensional image space. In the most general form, the observation model can be defined as $x_{\mathbf{i}}$ being a mapping of $y_{\mathbf{i}}$ through a function $\mathcal{F}$\todo{From where to where does $\mathcal{F}$ map? Also, this does not seem very general, since this $\mathcal{F}$ is limited to operate pixel-wise and cannot take into account the position $\mathbf{i}$ at all. It still may be general enough for what you need though. I don't know yet, since I just started reading.}:
\begin{equation}\label{eq:observation-model}
    x_{\mathbf{i}} = \mathcal{F} (y_{\mathbf{i}})
\end{equation}
where $x_{\mathbf{i}}$ and $y_{\mathbf{i}}$ represent the incomplete observation and the corresponding true image value at voxel $\mathbf{i}$, respectively; the entire image can be represented as $X = [x_{\mathbf{i}}]$ \todo{This kinds of reverses the logic. Before you can connect the observation $X$ to the true image $Y$, you first need to define exactly what the true image $X$ is supposed to be. From the above I would say it's a d-array of real values.} and $Y = [y_{\mathbf{i}}]$, respectively. And the function $\mathcal{F}$ can vary depending on the noise model (e.g., additive or multiplicative noise, etc.)\todo{Will you give examples for $\mathcal{F}$ later? I think it would already be a good place to have a very simple example right here, e.g. additive noise. Then we probably see, that this is not truly a function, but involves a random variable.}.

This mapping, however, is not injective \todo{To understand that, you should put an example $\mathcal{F}$ here. I'm not completely sure how you mean this exactly.} and therefore, the inverse problem\footnote{Referred to as image restoration, reconstruction or denoising} to estimate $y_{\mathbf{i}}$ from $x_{\mathbf{i}}$, is an \textit{ill-posed} problem, meaning that there are multiple possible solutions for $y_{\mathbf{i}}$ that could generate $x_{\mathbf{i}}$.

\todo{This short literature overview needs references that back your statements.}%
Image restoration techniques have been explored in both the spatial and transform domains\todo{Directly give an example for what transform domains are, e.g., the frequency domain.}. In the spatial domain, several linear and nonlinear filtering techniques exist, utilizing different kernels to perform direct manipulation on pixel values. Linear filters, such as the mean filter, compute the \todo{inserted ``weighted''} average of pixel values in a neighborhood and, in doing so, smooth the image but usually result in edge blurring.
...
The principle behind \gls{NLM} \todo{Since NLM is quite related to BM3D, it would also be helpful to define this in detail here. The definition is not that involved and I would say quite instructive to understand the non-locality.} denoising also \todo{``also''? What else uses this redundancy? BM3D, but you haven't mentioned that one yet. At leae not in this chapter.} builds on the redundancy of similar patches in the image and estimates a pixel value by taking a weighted average over other pixels with a similar local structure, regardless of their spatial distance. By utilizing these priors, denoising algorithms can indeed aggressively use the intrinsic properties of the images and thereby enhance the visual quality and provide estimates closer to the true image.

A lot of the mentioned algorithm assume the noise prior to be additive. This can be written as:
\begin{equation}
    X = Y + N
\end{equation}
\todo{This does not fit into your framework for the mapping $\mathcal{F}$, because this does not do the same at every pixel: At pixel $\mathbf{i}$, the noise $n_\mathbf{i}$ is added, but this depends on $\mathbf{i}$.}%
