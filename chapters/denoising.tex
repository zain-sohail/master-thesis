...
where $\text{Poi}(y_{\mathbf{i}})$ denotes the Poisson distribution with latent intensity $y_{\mathbf{i}}$. The \gls{PMF} of the observed value $x_{\mathbf{i}}$ is given by:

\begin{equation}
    P(x_{\mathbf{i}} = x| y_{\mathbf{i}} = y) = \frac{y^x e^{-y}}{x!}
\end{equation}

Poisson distribution has the key property that the mean and variance are equal, $\mathbb{E}[x_{\mathbf{i}}] = \text{Var}[x_{\mathbf{i}}] = y_{\mathbf{i}}$, with standard deviation $\sqrt{y_{\mathbf{i}}}$. In contrast to the \gls{AWGN} model, which assumes a variance independent of the true intensity, the Poisson noise model has variance scaling with the true intensity. This implies that with decreasing intensity, the fluctuations (standard deviation) increase. Consequently, \gls{AWGN} denoising algorithms, such as \gls{BM3D}, can not directly be applied to Poisson noise.

To address this issue, suitable transformations that stabilize the variance, making it largely independent of the mean (similar to the normal distribution) can be beneficial. Transforms such as these are known as \glsxtrfullpl{VST}. \citeauthor{bartlettSquareRootTransformation1936} \cite{bartlettSquareRootTransformation1936} first showed such a transform: the square root transformation $T_{\sqrt{\cdot}}$:
%Moreover, \glspl{VST} variance stabilization is independent of the sample size.

% More abstractly, let us consider a random variable $Z$ with a Poisson distribution, $Z \sim \text{Poi}(\lambda)$. The variance of $Z$ is given by $\mathbb{E}[X] = \text{Var}[Z] = \lambda$.

\begin{equation*}
    T_{\sqrt{\cdot}}: [0, \infty) \to [0, \infty), \quad Z \to \sqrt{Z}
\end{equation*}

where $Z$ is Poisson random variable with parameter $\lambda$.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{images/data_transform_anscombe.pdf}
    \caption{\textit{Variance stabilization through various transformations of Poisson-distributed data.} The plot shows the variance of Poisson-distributed random variables (blue) along with the variances of their transformed values under different variance-stabilizing transformations: the square root transform (orange, and scaled transform green), the square root transform with a constant of $\frac{1}{2}$ (red), and the Anscombe transform with a constant of $\frac{3}{8}$ (purple). 
    As the Poisson parameter $\lambda$ increases, the variance of the square root transform stabilizes to approximately $1$, with the fastest convergence using the Anscombe transform.
    The random variables are scaled by $2$ (except orange and blue), allowing the data to more closely approximate a standard normal distribution. Dashed horizontal lines indicate the stabilization levels.}
    \label{fig:data-transform-anscombe}
\end{figure}

Applying this transform to Poisson data makes the transformed values approximately normally distributed as the Poisson parameter $\lambda$ increases. The blue and orange line plots in \cref{fig:data-transform-anscombe} show the variance of $Z$ and its square root as a function of $\lambda$. It can be seen that while the variance of $Z$ increases with $\lambda$, the variance of $\sqrt{Z}$ stabilizes to $\frac{1}{4}$. \citeauthor{bartlettSquareRootTransformation1936} additionally showed that by adding a constant of $\frac{1}{2}$ to the square root transform, the convergence to normality improves, leading to the transformation:

\begin{equation*}
    T_{\sqrt{\cdot + \frac{1}{2}}}: [0, \infty) \to [0, \infty), \quad Z \to \sqrt{Z + \frac{1}{2}}
\end{equation*}

The red line plot \cref{fig:data-transform-anscombe} shows this faster convergence (also preventing overshoot) when compared to the green line plot. Here, the random variables are scaled by \num{2}, effectively centering the variance around \num{1}\footnote{The variance scales by square of the scaling factor, i.e., $\text{Var}[aZ] = a^2 \text{Var}[Z]$}, yielding a transformed variable that approximates a standard normal distribution.

\citeauthor{anscombeTransformationPoissonBinomial1948} \cite{anscombeTransformationPoissonBinomial1948} showed that the constant of $\frac{3}{8}$ is an optimal \gls{VST} $T_A$ for Poisson distributed data:

\begin{equation}
    T_A: [0, \infty) \to [0, \infty), \quad Z \to 2 \sqrt{Z + \frac{3}{8}}
\end{equation}

In \cref{fig:hist-anscombe}, the distribution is visualized before after applying the Anscombe transform with $\lambda$ values \numrange{1}{8} (with the mean value subtracted), highlighting how variance stabilization improves with increasing $\lambda$. Comparing $T_{\sqrt{\cdot + \frac{1}{2}}}$ (red) with $T_A$ (purple) in \cref{fig:data-transform-anscombe}, it can be seen that the variance of the Anscombe transform stabilizes to \num{1} faster, while not significantly different from the square root transform with a constant.


\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{images/hist_anscombe.pdf}
    \caption{\textit{Variance stabilization through the Anscombe transform.} The histogram shows the distribution of Poisson-distributed random variables with different $\lambda$ values, before and after applying the Anscombe transform. The data approaches standard normality as $\lambda$ increases.}
    \label{fig:hist-anscombe}
\end{figure}

The Anscombe transform can hence be applied to the corrupted image $X$ (to each voxel individually $x_{\mathbf{i}}$) to stabilize the variance, making it suitable for denoising using \gls{AWGN} denoising algorithms. After the denoising, an inverse transformation must be applied to recover the estimated image $\hat{Y}$.

\todo[disable]{Bad notation. $\mathbb{Z}$ is usually the set of integers.} is an estimate of expected value of $T(X)$ conditioned on the true signal $Y$ we aim to recover.\todo[inline]{I don't understand this part and what you do with the expectation here. Why is this more than just ``apply $T$, denoise, apply $T^{-1}$''?}
\begin{equation}
...
\end{equation}
Since the transformation is non-linear, an algebraic inverse is asymptotically biased\todo{I don't see how the non-linearity of the transformation should result in ``an'' algebraic inverse to be asymptocically biased. It's not even clear what you mean with the term ``asymptocically biased''. For the algebraic inverse, I guess you mean by the inverse you get by solving (3.8) for $X$.}.. \citeauthor{makitaloOptimalInversionAnscombe2011} \cite{makitaloOptimalInversionAnscombe2011} proposed an exact unbiased \todo{What does ``exact unbiased'' mean?} inverse of $\hat{Y}_{\text{bm3d}}$ giving the denoised estimate $\hat{Y}$. This method performs better than methods based on explicit Poisson noise removal. Therefore, \todo{So the $T^{-1}$ is the ``exact unbiased inverse'', not the ``algebraic inverse''? The notation ``$T^{-1}$'' means the inverse of $T$, which is unique. If you don't apply the inverse, explain what exactly you apply, use a different notation and if possible also give a formula.} in the next chapter we make use of the scheme described in \cref{alg:anscombe-bm3d} to denoise 2D image slices from the \gls{MPES} data.
...
\begin{algorithm}
    ...
    \Procedure{AnscombeBM3D}{$X$, $\sigma^2$}
        ...
        \State $Y_A \gets \textsc{BM3D}(X_A, \sigma^2)$
        
        \State $\hat{Y} \gets T^{-1}(Y_A)$
